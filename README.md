...
| 系统 | 包 | 大小 | 说明 |
|---|---|---|---|
| **Windows** | `wangpan-manager-V1.0.13-Windows.zip` | 约 466 MB | 解压即用（x64） |
| **Linux** | `wangpan-manager-V1.0.13-Linux.zip` | 约 905 MB | 解压即用（x86_64） |
...
**播放画面彻底归口 + 崩溃根治 + 全屏/窗口尺寸修复 + 截图透明提示**

### 🎬 画面只走一条路（"播放归口"）

以前"把画面交出去"散在播放页、独立窗口、画面自愈、AI 重载四五个地方，各写一套 ——
同一类 bug 换条路径就复发。现在只有 `v8_3/播放/播放出口.py` **一处**决定画面往哪画：
视频控件自己的 X11 子窗口 + `set_xwindow()`（libvlc 唯一会触发嵌入的 API）。

真机矩阵实测（本机 4K60 HEVC 直链 + vaapi，读的是 VLC 自己的日志）：

| 做法 | `vout window` 模块 | 游离窗口 |
|---|---|---|
| `set_xwindow(窗口)` | `"embed-xid,any"` ✓ | 无 ✓ |
| 实例级 `--drawable-xid`（V1.0.9 的做法） | `"any"` ✗ | **有** ✗ |
| 媒体级 `:vout=xxx` | 完全不生效（VLC 仍报 `"any"`） | — |

⇒ **只有 `set_xwindow` 会触发嵌入**；V1.0.9 改成实例级 drawable 的那一版，
恰好把嵌入关掉了（用户反馈的"更严重"就是这个）。

顺手修掉三处老问题：

- **画面与视频控件精确对齐**：以前自己另建 X11 子窗口再用 `mapTo(顶层窗口)` 算偏移 ——
  非原生控件的 `winId()` 返回的就是顶层窗口号，偏移必然差一个控件位置，
  于是画面歪到右下角还被裁。现在把视频控件设成 `WA_NativeWindow`，
  **位置/尺寸/映射全由 Qt 跟着布局走**（实测 X 窗口几何与控件完全一致）。
- **页面 ⇄ 独立窗口交接**：统一成"先换出口 → 停干净 → **等 vout 消失** → 换绑 → 重开 →
  跳回原位置"。以前一边往新窗口起播、一边句柄还指着即将销毁的旧窗口 ——
  关掉独立窗口又冒出 VLC 窗口就是这么来的；而且旧 vout 没释放就动它的画布会把 VLC 弄僵。
- **永远不销毁 VLC 正在画的窗口**（拆了它就会另开一个顶层窗口放画面）。

### 💥 播放崩溃：两个真病根（coredump 里读出来的）

- **`播放AI决策` 线程在 libvlc 里 SEGV**（`libvlc_media_add_option_flag`）：
  AI 顾问线程直接改播放参数（内部会重开媒体、加选项），界面线程同时在起播 ——
  libvlc 的 media/player 对象**不是线程安全的**。⇒ 会话里所有碰 libvlc / 碰 Qt 窗口的
  方法统一**排队给界面线程执行**（25ms 泵），并且**拿不到可嵌入窗口号时拒绝起播**。
- **`setAttribute(WA_NativeWindow)` 递归把自己压爆**（coredump 里 `createWinId`
  层层嵌套）：创建原生窗口会立刻派发事件，回调又进来问句柄，而标志位是"调用之后"才置位
  ⇒ 无限递归。现在先置位 + `testAttribute` 双重防重入，非界面线程一律不发窗口号。
- **定时器拖尾崩溃**：`QTimer.singleShot` 不持有宿主对象，窗口销毁后回调会落到已析构的
  C++ 对象上（coredump：`QTimerInfoList::activateTimers → notifyInternal2 → SEGV`）。
  新增 `v8_3/界面/定时.py` 的「安全单发」（定时器挂在宿主名下），
  播放页 / 独立窗口 / 主窗口 / 全屏助手全部换掉。

### 🖥 全屏与独立窗口尺寸

- **播放页的「⛶ 全屏」按钮以前完全没反应**，两个原因：① 槽函数里的分割器属性名写错
  （写的是 `self.分隔`，实际叫 `self.主体`）→ AttributeError 被 Qt 吞掉；
  ② 本机合成器（COSMIC）**忽略 `showFullScreen()`**。⇒ 新增 `v8_3/界面/全屏助手.py`
  作为唯一实现：请合成器全屏 → 0/120/400ms 复核 → **没铺满就自己铺满**；
  退出还原几何、Esc 也能退、按钮文字跟着状态走。
- **独立窗口退出全屏播 4K 时"窗口过长超出屏幕"**：合成器忽略客户端 `move()`，
  而且在我们挪完之后**再**自己摆一次 —— "按当前几何算明明在屏里"是假象。
  ⇒ **落点记忆**：记住它坚持的落点，之后任何尺寸计算都按那个落点收。
- 独立播放器窗口保证放得下四排控件（菜单栏 / 进度条那排 / 播放暂停那排 / 状态栏），
  最小尺寸 360×380，不会再被压没。

### 📷 截图会弹透明提示

截图后弹出**半透明浮层**（不挡画面、鼠标穿透、4.2 秒自动淡出）：绿色加粗的**图片名** +
下面一行**保存位置**（完整目录）。做法上试了四种才成：

| 做法 | 真机结果 |
|---|---|
| `QLabel` + 透明度特效 | 画不出来（只剩纯色块，文字全不见） |
| 普通子控件 | 被 VLC 的原生子窗口整个盖住 ✗ |
| 原生子窗口 | 能浮上去，但连累视频窗口重建 → 画面跑到 VLC 自己窗口 ✗ |
| **无边框 + 绕过窗口管理器 + 半透明顶层浮窗** | ✅ 位置我们说了算、必然浮在画面上、画面能透出来 |

### ✅ 真机验证（作者这台机器，用软件本身、播 4K60 直链）
