...
# （检查项全绿、退出码却是 139）—— 自检里一律用注入的假引擎，不建真 QtWebEngine。
os.environ.setdefault("V8_3_只用注入引擎", "1")
import tempfile
import time
...
def main() -> int:
    ...
    # 界面：卡片渲染 + 五个操作按钮
    市场页 = AI页
    # 自检要**确定性**：本机若真装过模型（ollama list 有），卡片会显示
    # 「✅ 已安装」，「一键安装」按钮就不在了 —— 实测在开发机上假失败过。
    # 做法：让"本机已装清单"在自检里为空（_填市场 会用 ollama list 覆盖状态，
    # 只压条目的 已安装 字段没用）。
    市场页.本地客户端 = lambda: None
    市场页._填市场(离线目录)
    泵(0.3)
