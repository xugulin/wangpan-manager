...
# QtWebEngine 在自检这种『反复建/销毁 profile』的脚本里退出时会段错误
# （检查项全绿、退出码却是 139）—— 自检里关掉预热（预热只是提速，不是功能）。
os.environ.setdefault("V8_3_不预热", "1")
import tempfile
import time
...
def main() -> int:
    ...
    try:
        ...
        # 预热登录页（提速）：假的造引擎 → 真窗口应"接管"它、不再重复加载
        from v8_3.界面 import 预热登录页 as _预热模块
        _造的: list = []

        def _假造引擎(_类型名, _配置, 父=None):
            _引 = _假引擎2()
            _引.名字 = "假引擎(预热)"
            _引.地址 = "about:blank"
            _引.页面地址 = lambda: _引.地址
            _造的.append(_引)
            return _引

        _预热模块.丢掉预热引擎()
        # 直接往池子里塞一个假引擎（不真开浏览器）——"接管 + 不重复加载"这两条
        # 逻辑与引擎实现无关，用假引擎验最干净。
        _假预 = _假造引擎(None, None)
        _假预.地址 = _预热模块.网盘入口网址("baidu", "sms")
        _预热模块._预热["baidu"] = {"引擎": _假预, "宿主": None, "地址": _假预.地址}
        _假预.打开(_假预.地址)
        _在池 = "baidu" in _预热模块.状态()["已预热"]
        检查(_在池 and not _预热模块.预热登录页("baidu"),
             "预热池在管：已有预热时不会重复起（"
             + str(_预热模块.状态()) + "）")
        _引0 = _假预
        _接管窗 = _内置窗口("baidu", None, 完成回调=lambda _c: None, 登录方式="sms")
        检查(getattr(_接管窗, "接管了预热", False) and _接管窗.引擎 is _引0,
             "登录窗口会接管预热好的引擎（页面已加载，不再等那几秒）")
        _打开次数 = len([x for x in _引0.操作记录 if x[0] == "打开"])
        检查(_打开次数 == 1,
             f"接管后不会重复加载页面（当前打开次数 {_打开次数}）")
        _接管窗.close()
        检查("baidu" not in _预热模块.状态()["已预热"],
             "登录窗口关掉后预热池已清空（不留后台进程）")
        # 预热页面"还在加载/是白板"时，接管要强制重开一次（用户实测踩过白板）
        _白引 = _假造引擎(None, None)
        _白引.页面地址 = lambda: "https://pan.baidu.com/disk/main"
        _白引.还在加载 = True
        _预热模块._预热["baidu"] = {"引擎": _白引, "宿主": None,
                                "地址": "https://pan.baidu.com/disk/main"}
        _白引.打开("https://pan.baidu.com/disk/main")
        _白窗 = _内置窗口("baidu", None, 完成回调=lambda _c: None)
        for _ in range(10):
            泵(0.02)
        _白次数 = len([x for x in _白引.操作记录 if x[0] == "打开"])
        检查(_白次数 == 2,
             f"预热页面还在加载时，接管会强制重开一次（打开次数 {_白次数}）")
        _白窗.close()
        _预热模块.关掉全部预热()
        # 自检必须在退出前把预热池彻底清空：池子里若留着真的 QtWebEngine 引擎，
        # 隐藏宿主控件销毁后那个视图就成了野指针 —— 实测会让自检进程**段错误**。
        _预热模块.关掉全部预热()
    ...
    try:
        ...
        检查(_短信方式 != "内置浏览器" or not 百度对话.发送验证码按钮.isEnabled(),
             ...
             + (_短信方式 or "无") + "）")
        # 无条件记一条（假网盘没声明机制，这条就自然成立）：
        # 规则本身要能被日志证明："声明了内置浏览器方式的后端，
        # 短信页不显示 手机号框/发码按钮/验证码框"
        _隐藏了 = all(
            (getattr(百度对话, _n, None) is None
             or not getattr(百度对话, _n).isVisible())
            for _n in ("手机号框", "发送验证码按钮", "验证码框", "短信登录按钮"))
        检查(_短信方式 != "内置浏览器" or _隐藏了,
             "内置浏览器方式的短信页不显示输入框与发码按钮（当前方式="
             + (_短信方式 or "无") + "）")
        if _短信面板 is not None and _短信方式 == "内置浏览器":
            # 用户要求：百度短信页**不要**手机号框/发码按钮/验证码框，
            # 只留「用内置浏览器跳到短信登录页」那一个按钮
            for _名, _键 in (("手机号框", "手机号框"), ("发送验证码按钮", "发送验证码按钮"),
                          ("验证码框", "验证码框"), ("短信登录按钮", "登录按钮")):
                _控件 = getattr(百度对话, _名, None)
                检查(_控件 is None or not _控件.isVisible(),
                     f"百度短信页不显示{_键}（用户要求去掉）")
            检查(getattr(百度对话, "短信内置浏览器按钮", None) is not None
                 and 百度对话.短信内置浏览器按钮.isVisible(),
                 "百度短信页保留『用内置浏览器跳转』按钮")
