...
**本次更新（{标签}）：Windows「界面奇卡无比」的真凶找到了 —— 界面线程在同步跑 ollama 子进程。**

真机 Windows runner 分段计时量到的原始数字：建主窗口 **54.5 秒**、切到播放页
**36.5 秒**、AI 页 **24.4 秒**、敏感词页 **12.3 秒**。修完同一台机器上：
建主窗口 **6.8~8.5 秒**、切播放页 **40 毫秒**、AI 页 **147 毫秒**、
Windows 上整套单测 **500 秒 → 69 秒**。

- **元凶**：列"本机装了哪些模型"时第一件事是 `ollama list`（起一个子进程、30 秒超时），
  AI 页构建时走了 3 次；"内置基座说明"又同步跑了一次 `ollama --version`。
  Windows 上**第一次**拉起没有签名的 `ollama.exe`，Defender 要先整个扫一遍（20 秒起），
  界面就这么被顶住 —— 而且窗口一显示就卡，用户看到的就是"Windows 版界面奇卡"。
- **修法**：界面线程只读缓存 / 读磁盘，绝不碰子进程与网络 ——
  已装模型改成直接读 ollama 仓库的 `manifests/` 清单（毫秒级，层没下全的不算已装）；
  基座版本号内存 + 磁盘缓存，后台线程预热一次；只有后台线程才允许真的跑 `ollama list`。

**主题样式表：把"全局那一层"从 QSS 换成 QPalette（界面再快 1.6~3 倍）**

- 先量：清掉整套 QSS 后切同样几页 12.3 秒 → **40 毫秒**，说明贵的是样式匹配；
  其中 `QWidget {{ 背景色; 文字色; 字号 }}` 这条规则**每个控件**都要沾一次。
- 现在背景/文字/选中色走 `QPalette`、字号走应用字体，QSS 里删掉这条全局规则。
  真机实测：建主窗口 **11.2 s → 6.8 s**、切播放页 **107 ms → 40 ms**。
- 视觉回归：8 个页面 × 1440×900 两档像素比对，差异仅 **0.16%~0.41%**（抗锯齿级），
  暗色（Dracula）与亮色（GitHub Light）两套主题肉眼一致。
  想退回老写法：设 `V8_3_主题全局层=样式表`。

**小分辨率屏幕的动态适配**

- AI 状态页在窗口不够宽时从"三列并排"自动改成"上下堆叠"——
  以前 1024×600 上右边的价格表会被窗口边缘裁掉，既看不见也滚不到；
  阈值按控件 `sizeHint` 算，Windows 字体更宽时自动抬高（1366×768 仍是三列，实测无裁切）。
- 高 DPI 缩放（1366×768 @150% → 911×512 逻辑分辨率）下，窗口尺寸与最小尺寸都不再超出屏幕。
- 窗口宽度变化会通知各页面重排；页面自己判断要不要动，拖动窗口不会反复重建布局。

**其他**：**「创建桌面图标」乱码/创建失败**已修（`创建桌面图标.vbs`：中文用 `ChrW()` 拼、
UTF-16LE 保存，彻底绕开 cmd 的代码页问题；Wine 实测生成的快捷方式名就是 **网盘管理**）；
AI 页顶部那个 `builtin | 内置兜底 | 3 个模型` 小徽标已删除（上一版资源是更早构建的，
所以还能看到）；播放时多出游离 VLC 窗口 / 关窗卡死的问题也已修掉。
