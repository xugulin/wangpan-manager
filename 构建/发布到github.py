...
**本次更新（{标签}）：下载 AI 模型不再弹两个大黑框（VIP 用户实测反馈）。**

Windows 版点「下载模型」时，屏幕上会冒出**两个关不掉的大黑框**，标题是
`…\运行环境\本地模型\ollama.exe` —— 一个是后台的 `ollama serve`，一个是 `ollama pull`。
它们是控制台程序，而 GUI 程序（`启动.exe`）起控制台程序时 **Windows 默认新开一个
控制台窗口**；关掉它就等于中断下载，体验很差。

修法：新增 `v8_3/进程.py` 作为**起子进程的唯一入口**，带上
`CREATE_NO_WINDOW`（明确不创建控制台）+ `STARTUPINFO(SW_HIDE)` 双保险，
所有相关调用全部归口：

- `ollama serve / pull / list / rm / --version`（就是那两个黑框的来源）；
- 适配器桥子进程、ffprobe/ffmpeg（媒体信息、字幕）、语音识别子进程、一键更新；
- 全项目**不再有裸 `subprocess.Popen/run`**（有单测做静态检查，防止写回去）。

真机验证（CI 的 Windows runner）：用 GUI 子系统的 `pythonw.exe` 当父进程，
裸起 → 子进程拿到控制台窗口（= 黑框）；走我们的入口 →
子进程 `GetConsoleWindow()==0`，**不会弹黑框**。

**上一版（V1.0.11）的内置 VLC 也在这一版里**：Windows 包自带官方 VLC 3.0.23
运行时（`libvlc.dll` + 354 个插件，约 131 MB），解压即用、不用自己装 VLC；
打包脚本会强制校验（缺 libvlc 或插件不足就拒绝出包），CI 也会在**没装 VLC 的干净
runner** 上验证"包内自带的那份能用"。
