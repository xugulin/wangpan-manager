...
- **现在**：第一步就隐藏窗口（点 × 立刻就没）；等待预算收到 1 秒 + 0.6 秒；
  还没停的线程从窗口上「摘下来」（不销毁、不回收）交给进程退出收掉；
  事件循环结束后再给 1 秒宽限，仍在跑的直接结束进程 —— 不会留下
  "窗口关了但进程还在任务管理器里"。
...
- 实测：本地 `close()` **26 秒 → 1.6 秒**；Wine（真 Windows 运行时）复测
  `close秒 1.62、关窗后可见 false`；新增 4 条回归测试钉住。

**V1.0.7 的修复也都在这一版里**（V1.0.7 发布后才发现上面这个问题，故直接出 V1.0.8）：

- **Windows「界面奇卡无比」的真凶**：界面线程在同步跑 `ollama list` / `ollama --version`
  （Windows 第一次拉起没签名的 exe 要等 Defender 扫完，20 秒起），AI 页构建时走了 4 次。
  改成"界面线程只读缓存/读磁盘"，实测建主窗口 **54.5s → 6~8.5s**、切播放页 **36.5s → 40ms**、
  AI 页 **24.4s → 147ms**、Windows 上整套单测 **500s → 60s**。
- **主题全局规则换成 QPalette**（`QWidget {{ 背景色; 文字色; 字号 }}` 这条每个控件都要沾一次）：
  真机再快 1.6~3 倍（建主窗口 11.2s → 6.8s、切播放页 107ms → 40ms），
  8 个页面像素比对差异仅 0.16%~0.41%；想退回老写法设 `V8_3_主题全局层=样式表`。
- **小分辨率适配**：AI 状态页宽度不够时自动从三列改成上下堆叠（1024×600 上价格表不再被裁），
  高 DPI 缩放下窗口尺寸与最小尺寸都不超出屏幕。
- **「创建桌面图标」在真 Windows 上是真的 .lnk 了**：WSH 的 `TargetPath` 按 ANSI 代码页处理
  中文路径（英文版 Windows 上留下没有目标的空壳 .lnk），现在用 Shell32 原生 Unicode 接口
  写目标并**读回核对**，建不出来再降级 PowerShell + IShellLinkW；桌面图标名就是 **网盘管理**。
- AI 页顶部那个 `builtin | 内置兜底 | 3 个模型` 小徽标已删除。
