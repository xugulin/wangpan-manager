...
**更新内容（{标签}）**：**百度网盘支持短信登录**（点一下打开内置浏览器并直接跳到
短信登录页，登录成功自动收走会话、自动关窗）；修掉"登录成功却报已捕获 0 条 cookie"
的取早问题；百度短信页去掉用不了的发码输入框；运行环境再精简（Linux 861 MB /
Windows 429 MB）；打包抹掉构建机私有路径；新增 `工具/清登录数据.py`。
