...
**本次更新（{标签}）：播放画面彻底归口 + 崩溃根治 + 全屏/窗口尺寸修复 + 截图透明提示。**

### 🎬 画面只走一条路（播放"归口"）

以前"把画面交出去"散在播放页、独立窗口、画面自愈、AI 重载四五个地方，各写一套 ——
同一类 bug 换条路径就复发。现在只有 `v8_3/播放/播放出口.py` 一处决定画面往哪画：
视频控件自己的 X11 子窗口 + `set_xwindow()`（libvlc 唯一会触发嵌入的 API）。
真机矩阵实测（你机器上的 4K60 直链 + vaapi）：

| 做法 | vout window 模块 | 游离窗口 |
|---|---|---|
| `set_xwindow(窗口)` | `"embed-xid,any"` ✓ | 无 ✓ |
| 实例级 `--drawable-xid`（V1.0.9 用的） | `"any"` ✗ | **有** ✗ |
| 媒体级 `:vout=xxx` | 完全不生效 | — |

顺手修掉三处相关的老问题：**画面与视频控件精确对齐**（以前自己建窗口算偏移，
非原生控件的 `winId()` 是顶层窗口号 → 画面歪到右下角还被裁）、
**页面 ⇄ 独立窗口交接**（先换出口 → 停干净 → 等 vout 消失 → 换绑 → 重开 → 跳回原位置；
以前一边起播一边还指着即将销毁的旧窗口 → 关掉独立窗口又冒出 VLC 窗口）。

### 💥 播放崩溃：两个真病根（都是 coredump 里读出来的）

- **`播放AI决策` 线程在 libvlc 里 SEGV**：AI 顾问线程直接改播放参数（内部会重开媒体、
  加选项），界面线程同时在起播 —— libvlc 的 media/player 对象不是线程安全的。
  ⇒ 现在会话里所有碰 libvlc / 碰 Qt 窗口的方法**统一排队给界面线程执行**
  （25ms 泵），并且**拿不到可嵌入窗口号时拒绝起播**，绝不把 0 递给 libvlc。
- **`setAttribute(WA_NativeWindow)` 递归把自己压爆**（`createWinId` 层层嵌套 → 栈溢出）：
  创建原生窗口会派发事件、回调又进来问句柄。⇒ 先置位 + `testAttribute` 双重防重入，
  并且**非界面线程一律不发窗口号**。

另外排掉一个隐蔽崩溃源：`QTimer.singleShot` **不持有宿主对象**，窗口销毁后回调会落到
已析构的 C++ 对象上（coredump：`QTimerInfoList::activateTimers → notifyInternal2 → SEGV`）。
新增 `v8_3/界面/定时.py` 的「安全单发」（定时器挂在宿主名下），播放页 / 独立窗口 /
主窗口 / 全屏助手全部换掉。

### 🖥 全屏与独立窗口尺寸

- **播放页的「⛶ 全屏」按钮以前完全没反应**：① 槽函数里的分割器属性名写错
  （`self.分隔`，实际叫 `self.主体`）→ AttributeError 被 Qt 吞掉；② 这台机器的合成器
  （COSMIC）**忽略 `showFullScreen()`**。⇒ 新增 `v8_3/界面/全屏助手.py` 作为唯一实现：
  请合成器全屏 → 0/120/400ms 复核 → **没铺满就自己铺满**；退出时还原几何、Esc 也能退、
  按钮文字跟着状态走。
- **独立窗口退出全屏播 4K 时"窗口过长超出屏幕"**：合成器忽略客户端 `move()`，还在我们
  挪完之后自己再摆一次 —— "按当前几何算明明在屏里"是假象。⇒ **落点记忆**：记住它坚持的
  落点，之后任何尺寸计算都按那个落点收，不管它把我们放哪都完整可见。
- 独立播放器窗口保证放得下四排控件（菜单栏 / 进度条那排 / 播放暂停那排 / 状态栏），
  最小尺寸 360×380 —— 不会再被压没。

### 📷 截图会弹透明提示

截图后弹出一个**半透明浮层**（不挡画面、鼠标穿透、4.2 秒自动淡出），
绿色加粗显示**图片名**、下面一行是**保存位置**（完整目录）。
做法上试了四种才成：QLabel+透明度特效（画不出来）、普通子控件（被 VLC 的原生子窗口盖住）、
原生子窗口（会连累视频窗口重建 → 画面跑到 VLC 自己窗口）、
**无边框 + 绕过窗口管理器 + 半透明顶层浮窗**（现在这版，位置我们说了算、必然浮在画面上）。

### ✅ 真机验证（就在作者这台机器上，用软件本身、播 4K60 直链）

- 播放页全屏 / Esc 退出 / 独立窗口全屏 / 退出全屏 —— 全部 ✅，退出后**继续播放**；
- 压力：4 轮参数重载（AI 线程故意在起播瞬间插一脚）+ 暂停/继续/跳转 + **泡机 60 秒**
  → **无游离窗口、全程不崩溃**；修复后**没有再产生任何 coredump**；
- 软件自带界面自检 **510/510 通过**；单测 **720 条全绿**（连跑 3 次）。
...
| 你的系统 | 下载这个 | 说明 |
|---|---|---|
| Windows 10/11 | `wangpan-manager-{标签}-Windows.zip` | 约 466 MB：不含 AI 语音模型，首次用字幕时自动联网下载 |
| Linux x86_64 | `wangpan-manager-{标签}-Linux.zip` | 约 905 MB：不含 AI 语音模型（同上） |

> 资源名用英文：GitHub 会把中文文件名清洗掉（只剩 `-V<版本>-Linux.zip`），
> 所以上传后统一改名成 `wangpan-manager-V<版本>-<平台>.zip`；
> `SHA256SUMS.txt` 里的名字与此一致。

> **本项目只发布「不含模型」的包**：包体小、下载快。AI 语音识别模型首次用字幕时
> 自动联网下载；本地大模型在「🤖 AI → 🛒 模型商店」一键安装；**ollama 基座（官方便携版
> 运行时）已经内置在包里**，解压即用，点「⬆️ 更新内置ollama」可升到官方最新版。

**用法**：解压 → 双击 `启动.exe`（Windows）或 `启动.sh`（Linux）→ 在网盘页点
「登录 / 管理」：百度可用「扫码」或「短信（内置浏览器跳转）」；夸克、光鸭用「扫码」。
