...
**本次更新（{标签}）：播放时又冒出 "VLC media player" 独立窗口 —— 挖到真病根并治本。**

前几轮都在治症状（拦掉 `:vout=gl`、起播后巡检再纠正），所以总是复发。这次用真 VLC
把日志一句句读出来（`cvlc -vvv` + Xvfb）：

- **媒体级 `:vout=` 根本不生效**：`cvlc ":vout=xcb_x11"` 里 VLC 依旧 `matching "any"` → 用 `gl`；
  只有实例级 `--vout=xcb_x11` 才会 `using "xcb_x11"`。以前"给 4K60 加 `:vout=gl`"、
  "嵌入时把 `:vout=gl` 剔掉"，改的都是**没人看**的选项。
- **实例级 `--vout` 会被 `set_xwindow()` 重置**（嵌入只有这一条 API）⇒ 两者不可兼得。
- **真病根**：给了窗口号后，VLC 用 `"embed-xid,any"` 找 vout **window** 模块 ——
  优先嵌进我们的窗口，**尝试失败就退到 "any"：自己开一个顶层窗口，而且不报错**。
  失败的现实原因：① 窗口还没真的映射（Qt 说"可见"≠ X 已 map）；
  ② 交出去的是 **Qt 的窗口**（合成器/主题下常是 ARGB/depth 32），visual 不匹配。

**治本**：不再把 Qt 的窗口交给 VLC，而是**我们自己用 Xlib 建一个 24 位 TrueColor 的朴素
子窗口**、自己 map 并等到 `IsViewable` 再交出去。映射与 visual 都由我们保证，
VLC 的 embed 尝试就不可能失败 → 那条"自己开窗口"的路走不到。

真 X11 实测（走应用真实代码路径，连播 3 次 + 一次缩放）：
交给 Qt 窗口 → 有时落到 "any" 自开窗口 ✗；交给自建子窗口 → **3/3 全部嵌住**、
缩放后不游离、收尾无残留 ✓。

一并修掉三个"复发帮凶"：
① **任何一次重播都先"停止并等待"**（以前只在窗口号变化时才等，旧 vout 没释放就起新的
→ 新 vout 因 drawable 正忙而自己开窗口）；实例级目标变了就重建实例。
② **巡检改成整个播放会话都在盯**，连续 60 秒干净就**复位回退阶梯**
（以前会话级只增不减，用完两次就再也不救 —— 这就是"同一个毛病反复出现"的直接原因）。
③ 回退阶梯按病因重排：先关 GPU 解码（VAAPI 要用 GL 输出，正是会自开窗口那条路），
再换 `xcb_xv` 输出；永远不销毁 libvlc 的窗口（那会把 VLC 弄僵、界面卡死）。

新增回归测试：嵌入宿主单测 + 真 X11 集成测试（连播 3 次 + 缩放都不许出现游离窗口）。
709 条测试在 Xvfb 下全绿；Wine 冒烟确认 Windows 上该模块是无害空操作。

**V1.0.8 / V1.0.7 的修复也都在这一版里**：点 × 立即退出（26 秒 → 1.6 秒）、Windows
界面卡顿（建主窗口 54.5s → 6~8s；切页 36.5s → 40ms）、主题全局层换 QPalette（再快 1.6~3 倍）、
小分辨率动态适配（AI 状态页窄屏自动堆叠）、桌面快捷方式在真 Windows 上真的建成
`网盘管理.lnk`（WSH 的 ANSI 坑）、AI 页顶部小徽标已删除。
