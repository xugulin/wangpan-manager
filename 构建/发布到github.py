...
**更新内容（{标签}）**：**修「创建桌面图标」乱码/创建失败** —— 用 Wine 实测定位到
真原因：cmd 的 `if exist "中文名"` 是按控制台代码页做**字节比较**的，中文文件名必然失配
（连 `%HERE%` 展开的项目路径都带中文），所以旧脚本永远找不到启动器；PowerShell 又可能
被策略禁用。现在改用 **`创建桌面图标.vbs`**（cscript + 中文用 ChrW() 拼 + UTF-16LE 保存）
加纯 ASCII 的 .bat 转发器（通配符找文件），并在最坏情况下把启动器复制到桌面兜底；
包里也新增了纯 ASCII 的 `启动.bat`。另外：**修 Windows 实机反馈的两件事** ——
①「创建桌面图标.bat」终端乱码且创建失败：该 .bat 现在**只用 ASCII**（中文快捷方式名
由 PowerShell 用码点拼），并且包里新增 **`启动.bat`** 兜底（旧脚本写死了包里可能
不存在的 `启动.exe`）；②界面卡顿：新增 **`启动.py --性能诊断`**，会抓"卡住时主线程
在哪一行"写进 `数据/卡顿诊断.log`（含系统/Python/平台插件/屏幕缩放倍数），
不再靠猜。另外：**修「播放时多出一个 VLC media player 窗口」** ——
根因是 libvlc 拿到"还没映射到屏幕"的窗口号就自己开顶层窗口，而 Qt 说"可见"
不等于 X 已映射；而且程序里两个播放窗口各写了一套判断（修一个漏一个）。
现在判断收敛成唯一入口（以 X 的 map_state 为准）、起播前轮询等窗口上屏、
等不到就先不播，游离子窗口也会连子窗口一起清掉。
另外：**修「一键启动点了没反应」** —— 启动脚本现在放在项目里、
项目外都能找到项目；过期/串号的 PID 文件不再锁死启动（会核对进程命令行）；
新增 `数据/一键启动.log` 启动日志、`一键启动（看报错）.desktop` 和 `使用说明.txt`。
同时**删掉了上一版引入的「预热登录页」** —— 实测它会让登录窗口一片空白，
而且量下来没有可量化收益（窗口自己加载：首次约 3 秒、缓存热了 0.2 秒以内）；
保留了防白板兜底（页面空白时自动重载一次）。
...
| 你的系统 | 下载这个 | 说明 |
|---|---|---|
| Windows 10/11 | `wangpan-manager-{标签}-Windows.zip` | 约 429 MB：不含 AI 语音模型，首次用字幕时自动联网下载 |
| Linux x86_64 | `wangpan-manager-{标签}-Linux.zip` | 约 861 MB：不含 AI 语音模型（同上） |
