...
**本次更新（{标签}）：Windows 版内置 VLC —— 修掉"装完不能播放视频"这个致命问题。**

Windows 用户解压 V1.0.10 点播放时，播放页直接报
`libvlc 不可用：找不到 libvlc（VLC 的运行库）` —— 因为 **Windows 不像桌面 Linux 自带 VLC**，
而上一版没有内置，于是"直接在线看 4K 视频"这个核心功能在 Windows 上等于没有。现在：

- **包内自带官方 VLC 3.0.23（x64）运行时**：`运行环境/vlc/` 下含 `libvlc.dll`、
  `libvlccore.dll` 与 **354 个插件**（解码 / HTTP / 输出等，约 131 MB），解压即用，
  不用你自己装 VLC、也不需要管理员权限；
- 播放时**优先用包内这一份**：加载前把该目录加进 DLL 搜索路径，并把 `VLC_PLUGIN_PATH`
  指向包内 `plugins`（Windows 的 DLL 搜索**不含**被加载 DLL 自己的目录 ——
  只加载不设插件路径就会"能起来、不能播"）；找不到才回退系统 VLC / `V8_3_LIBVLC`；
- 随包附 VLC 的 `COPYING.txt` / `AUTHORS.txt`（GPLv2+ / LGPLv2.1+ 要求）；
- **打包脚本强制校验**：缺 `libvlc.dll` 或插件不足 50 个就**拒绝出包**；
  CI 里加了"干净 Windows runner（没装 VLC）上必须可用"的检查，不会再发出不能播的包。

Linux 包维持不变（桌面发行版基本都自带 VLC，找不到时按提示装即可）。

**顺便把上一版（V1.0.10）的东西也都在这一版里**：播放画面归口为唯一实现
（`set_xwindow` 是 libvlc 唯一会触发嵌入的 API，删掉了那条"实例级 drawable"的错路）、
画面与视频控件精确对齐、页面 ⇄ 独立窗口交接统一（先换出口 → 停干净 → 等 vout 消失 →
换绑 → 重开 → 跳回位置）、**两个崩溃真病根**（AI 决策线程碰 libvlc 的 SEGV；
`setAttribute(WA_NativeWindow)` 递归压爆栈；外加 `QTimer.singleShot` 拖尾崩溃 → 安全单发）、
播放页「⛶ 全屏」按钮无效与 Esc 退出、独立窗口退出全屏播 4K 时超出屏幕、
以及**截图透明提示**（半透明浮层显示图片名 + 保存位置）。
