...
自动联网下载。所以本脚本默认 ``--口味 精简``。

**内置 ollama 基座（用户要求）：发布包默认自带官方便携版 ollama** ——
只放运行时（基座），**不放任何模型权重**：任何用户解压就能用，不必自己装 ollama、
不需要管理员权限，装模型之前也不用先联网。它躺在 ``运行环境/本地模型`` 里，
代码按**项目相对路径**找（:func:`v8_3.AI.本地模型.项目内可执行文件`），
所以整个文件夹搬到哪儿都能跑。确实不想要它时加 ``--不要ollama``。
...
def 裁掉GPU后端() -> None:
    """发布包只带 CPU / Vulkan 推理后端。

    官方整包 2.1 GB 里 CUDA 占 ~2 GB（cuda_v12 + cuda_v13），而 GitHub Release
    单个资源上限是 2 GiB —— 不裁的话 Linux 包直接顶到上限、只能走分卷。
    裁掉后端不影响 CPU 推理（ollama 自己挑剩下的后端）。
    """
    sys.path.insert(0, str(项目根))
    try:
        from v8_3.AI.本地模型 import 精简推理后端
        省了, 裁了 = 精简推理后端()
    except Exception as e:  # noqa: BLE001
        说(f"  ⚠️ 裁剪 GPU 后端失败（不影响打包，只是包会偏大）："
          f"{type(e).__name__}: {e}")
        return
    if 裁了:
        说(f"  · 已裁掉 GPU 后端 {'/'.join(裁了)}"
          f"（省 {省了 / 1073741824:.2f} GB，只留 CPU/Vulkan）")
    else:
        说("  · 只有 CPU/Vulkan 后端（没有可裁的 GPU 后端）")


def 备好本地模型运行时() -> None:
    """确保 ``运行环境/本地模型`` 里有官方便携版 ollama 基座（打包会带进发布包）。

    用户要求：发布包**内置基础 ollama**（任何用户开箱即用），但**不含任何模型权重**。
    * 项目里已经有就直接跳过（幂等，正常情况就是这一条）；
    * 真的没有（比如刚 clone 下来、或被 ``--不要ollama`` 建过一次）才去官方下一份；
    * 参数 ``--不要ollama`` 可整体跳过（离线构建/不想让包变大时用）。

    注意路径：放进 ``运行环境/本地模型`` 是靠**项目相对路径**找的
    （:func:`v8_3.AI.本地模型.项目内可执行文件`），所以包解压到哪儿都能跑。
    """
    目标 = 项目根 / "运行环境" / "本地模型"
    名字 = "ollama.exe" if os.name == "nt" else "ollama"
    现成 = [目标 / 名字, 目标 / "bin" / 名字]
    if any(x.is_file() for x in 现成):
        说("  · 内置 ollama 基座已在 运行环境/本地模型（跳过下载）")
        裁掉GPU后端()
        return
    说("  · 项目里没有内置 ollama，现去官方下一份（约 1.4 GB，只放运行时，不含模型权重）…")
    sys.path.insert(0, str(项目根))
    try:
        ...
        说("     发布包里将不含 ollama；用户可在 AI 页点「⬆️ 更新内置ollama」补装。")
        return
...
def main() -> int:
    ...
    # 用户口径（长期）：发布包**默认内置 ollama 基座**（官方便携版，只放运行时，
    # 不含任何模型权重）——"任何用户都能用"。确实不想要时才加 --不要ollama。
    解析.add_argument("--不要ollama", action="store_true",
                    help="不预装内置 ollama 基座（默认预装运行时，但不含任何模型权重）")
    解析.add_argument("--预装ollama", action="store_true",
                    help="（已无需指定）内置 ollama 现在是默认行为，留着只为兼容旧命令")
    参数 = 解析.parse_args()
    ...
    if 参数.不要ollama:
        说("按参数要求：发布包不带内置 ollama 基座"
          "（AI 页的「⬆️ 更新内置ollama」仍可随时补一份）")
    else:
        说("准备内置 ollama 基座（官方便携版运行时，不含任何模型权重）")
        备好本地模型运行时()
    for 平台 in 平台们:
