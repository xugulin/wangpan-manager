...
class 工具栏自适应测试(unittest.TestCase):
    ...
    def test_任何宽度都尽量装得下(self):
        ...
        "装得下"有个**平台物理下限**：把音量滑条/循环/随机/上一个/下一个全收掉之后，
        剩下的按钮按本机字体量出来的宽度，就是工具栏再也压不下去的宽度 ——
        真机 windows-latest 上是 **726px**（Linux 上约 700px；中文字体 + emoji 更宽）。
        比它还窄的窗口只能靠页面的横向滚动区兜底（`_放进堆叠` 套的那层），
        所以判据是"要么真的装得下，要么已经压到本机下限"。
        """
        from v8_3.界面.vlc风格 import 构建工具栏
        栏 = 构建工具栏(self._动作表())
        栏.按宽度自适应(700)
        最紧需要 = int(栏.sizeHint().width())
        for 宽 in (700, 900, 1084, 1183, 1188, 1400, 1600, 1920):
            模式 = 栏.按宽度自适应(宽)
            需要 = 栏.sizeHint().width()
            self.assertLessEqual(
                需要, max(宽, 最紧需要),
                f"{宽}px 窗口里工具栏装不下（需要 {需要}px，本机最紧档 {最紧需要}px）")
        # 窄窗口：**先收次要入口，而不是急着掉到图标档**。
        # 用户明确要求「速度 / 🗂 面板」要有文字，而那四个（循环/随机/上一个/下一个）
        # 在右键菜单和菜单栏里都还有 —— 所以 800px 也该**尽量**保住文字档。
        #
        # ⚠️ 但"尽量"有个物理上限：字体宽度是平台决定的。真机 windows-latest 上
        # 精简档本身就要 1114px（Linux 只要 ~700px），800px 的窗口里怎么收都放不下，
        # 这时掉到图标档才是**正确**行为（先前这条断言在 CI 上红了：'图标' not in
        # ('精简','完整')）。所以判据改成"确认精简档真的塞不下"，而不是硬要文字档。
        栏.按宽度自适应(800)
        self.assertLessEqual(栏.sizeHint().width(), 800,
                             f"800px 里工具栏必须装得下（现在 {栏.sizeHint().width()}px）")
        if 栏.当前模式 not in ("精简", "完整"):
            栏.设置模式("精简", 1920)
            精简需要 = 栏.sizeHint().width()
            self.assertGreater(
                精简需要, 800,
                f"精简档只要 {精简需要}px，800px 的窗口就该保住文字档，不该掉到"
                f"{栏.当前模式}档")
            栏.按宽度自适应(800)
        # 图标档下按钮文字就是那个图标（"🗂"），"面板"两个字只在文字档里 ——
        # 所以这里只能要求"按钮还在"，文字要求留给 1183px 那条（用户实际窗口宽度）。
        if 栏.当前模式 in ("精简", "完整"):
    ...
    def test_反复切换宽度不会卡死(self):
        ...
        栏 = 构建工具栏(self._动作表())
        栏.按宽度自适应(700)
        最紧需要 = int(栏.sizeHint().width())      # 本机字体下压不下去的那个宽度
        for 宽 in (1600, 700, 1600, 700, 1183, 900, 1500):
            模式 = 栏.按宽度自适应(宽)
            self.assertLessEqual(栏.sizeHint().width(), max(宽, 最紧需要))
